...
type config struct {
	...
	logs      []map[int]interface{} // copy of each server's committed entries  数组下标是哪个raft，然后value是一个map，map的key表示
	// log 的index位置，value是这个index位置对应的日志是什么
	lastApplied []int
...
func (cfg *config) applier(i int, applyCh chan ApplyMsg) {
	for m := range applyCh {
		if m.CommandValid == false {
			...
			cfg.mu.Unlock()
			// 判断日志的提交是否是连续的，如果CommandIndex大于1的话，如果有不是连续的日志的话，那么prevok就是false。
			// 如果m.CommandIndex是等于1的话，我们的prevok也不是false，因为有个dummy log。
...
func (cfg *config) start1(i int, applier func(int, chan ApplyMsg)) {
	...
	// 由于这里都是新建的channel，所以不会担心之前的会有影响。
